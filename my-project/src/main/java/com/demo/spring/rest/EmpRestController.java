...
public class EmpRestController {
	...
	@PostMapping(path = "/save", produces = {MediaType.TEXT_XML_VALUE}, consumes = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<String> saveEmp(@RequestBody Emp e){
		if (repo.existsById(e.getEmpId())) {
			return ResponseEntity.ok("Employee eexists with id "+ e.getEmpId());
		}else {
			repo.save(e);
